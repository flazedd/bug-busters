# Test cases automatically generated by Pynguin (https://www.pynguin.eu).
# Please check them before you use them.
import pytest
import status as module_0


def test_case_0():
    int_0 = 3342
    exit_status_0 = module_0.http_status_to_exit_status(int_0)
    assert exit_status_0 == module_0.ExitStatus.SUCCESS
    assert module_0.ExitStatus.SUCCESS == module_0.ExitStatus.SUCCESS
    assert module_0.ExitStatus.ERROR == module_0.ExitStatus.ERROR
    assert module_0.ExitStatus.ERROR_TIMEOUT == module_0.ExitStatus.ERROR_TIMEOUT
    assert module_0.ExitStatus.ERROR_HTTP_3XX == module_0.ExitStatus.ERROR_HTTP_3XX
    assert module_0.ExitStatus.ERROR_HTTP_4XX == module_0.ExitStatus.ERROR_HTTP_4XX
    assert module_0.ExitStatus.ERROR_HTTP_5XX == module_0.ExitStatus.ERROR_HTTP_5XX
    assert (
        module_0.ExitStatus.ERROR_TOO_MANY_REDIRECTS
        == module_0.ExitStatus.ERROR_TOO_MANY_REDIRECTS
    )
    assert module_0.ExitStatus.PLUGIN_ERROR == module_0.ExitStatus.PLUGIN_ERROR
    assert module_0.ExitStatus.ERROR_CTRL_C == module_0.ExitStatus.ERROR_CTRL_C
    exit_status_1 = module_0.http_status_to_exit_status(int_0)
    assert exit_status_1 == module_0.ExitStatus.SUCCESS


def test_case_1():
    bool_0 = False
    exit_status_0 = module_0.http_status_to_exit_status(bool_0)
    assert exit_status_0 == module_0.ExitStatus.SUCCESS
    assert module_0.ExitStatus.SUCCESS == module_0.ExitStatus.SUCCESS
    assert module_0.ExitStatus.ERROR == module_0.ExitStatus.ERROR
    assert module_0.ExitStatus.ERROR_TIMEOUT == module_0.ExitStatus.ERROR_TIMEOUT
    assert module_0.ExitStatus.ERROR_HTTP_3XX == module_0.ExitStatus.ERROR_HTTP_3XX
    assert module_0.ExitStatus.ERROR_HTTP_4XX == module_0.ExitStatus.ERROR_HTTP_4XX
    assert module_0.ExitStatus.ERROR_HTTP_5XX == module_0.ExitStatus.ERROR_HTTP_5XX
    assert (
        module_0.ExitStatus.ERROR_TOO_MANY_REDIRECTS
        == module_0.ExitStatus.ERROR_TOO_MANY_REDIRECTS
    )
    assert module_0.ExitStatus.PLUGIN_ERROR == module_0.ExitStatus.PLUGIN_ERROR
    assert module_0.ExitStatus.ERROR_CTRL_C == module_0.ExitStatus.ERROR_CTRL_C


def test_case_2():
    pass


def test_case_3():
    bool_0 = False
    exit_status_0 = module_0.http_status_to_exit_status(bool_0)
    assert exit_status_0 == module_0.ExitStatus.SUCCESS
    assert module_0.ExitStatus.SUCCESS == module_0.ExitStatus.SUCCESS
    assert module_0.ExitStatus.ERROR == module_0.ExitStatus.ERROR
    assert module_0.ExitStatus.ERROR_TIMEOUT == module_0.ExitStatus.ERROR_TIMEOUT
    assert module_0.ExitStatus.ERROR_HTTP_3XX == module_0.ExitStatus.ERROR_HTTP_3XX
    assert module_0.ExitStatus.ERROR_HTTP_4XX == module_0.ExitStatus.ERROR_HTTP_4XX
    assert module_0.ExitStatus.ERROR_HTTP_5XX == module_0.ExitStatus.ERROR_HTTP_5XX
    assert (
        module_0.ExitStatus.ERROR_TOO_MANY_REDIRECTS
        == module_0.ExitStatus.ERROR_TOO_MANY_REDIRECTS
    )
    assert module_0.ExitStatus.PLUGIN_ERROR == module_0.ExitStatus.PLUGIN_ERROR
    assert module_0.ExitStatus.ERROR_CTRL_C == module_0.ExitStatus.ERROR_CTRL_C
    exit_status_1 = module_0.http_status_to_exit_status(exit_status_0)
    assert exit_status_1 == module_0.ExitStatus.SUCCESS
    bool_1 = True
    exit_status_2 = module_0.http_status_to_exit_status(bool_1)
    assert exit_status_2 == module_0.ExitStatus.SUCCESS
    int_0 = 386
    exit_status_3 = module_0.http_status_to_exit_status(int_0, exit_status_2)
    assert exit_status_3 == module_0.ExitStatus.ERROR_HTTP_3XX
    exit_status_4 = module_0.http_status_to_exit_status(exit_status_0)
    assert exit_status_4 == module_0.ExitStatus.SUCCESS


@pytest.mark.xfail(strict=True)
def test_case_4():
    bool_0 = False
    exit_status_0 = module_0.http_status_to_exit_status(bool_0)
    assert exit_status_0 == module_0.ExitStatus.SUCCESS
    assert module_0.ExitStatus.SUCCESS == module_0.ExitStatus.SUCCESS
    assert module_0.ExitStatus.ERROR == module_0.ExitStatus.ERROR
    assert module_0.ExitStatus.ERROR_TIMEOUT == module_0.ExitStatus.ERROR_TIMEOUT
    assert module_0.ExitStatus.ERROR_HTTP_3XX == module_0.ExitStatus.ERROR_HTTP_3XX
    assert module_0.ExitStatus.ERROR_HTTP_4XX == module_0.ExitStatus.ERROR_HTTP_4XX
    assert module_0.ExitStatus.ERROR_HTTP_5XX == module_0.ExitStatus.ERROR_HTTP_5XX
    assert (
        module_0.ExitStatus.ERROR_TOO_MANY_REDIRECTS
        == module_0.ExitStatus.ERROR_TOO_MANY_REDIRECTS
    )
    assert module_0.ExitStatus.PLUGIN_ERROR == module_0.ExitStatus.PLUGIN_ERROR
    assert module_0.ExitStatus.ERROR_CTRL_C == module_0.ExitStatus.ERROR_CTRL_C
    exit_status_1 = module_0.http_status_to_exit_status(exit_status_0)
    assert exit_status_1 == module_0.ExitStatus.SUCCESS
    bool_1 = True
    exit_status_2 = module_0.http_status_to_exit_status(bool_1)
    assert exit_status_2 == module_0.ExitStatus.SUCCESS
    bool_2 = False
    int_0 = 694
    exit_status_3 = module_0.http_status_to_exit_status(int_0, exit_status_2)
    assert exit_status_3 == module_0.ExitStatus.SUCCESS
    exit_status_4 = module_0.http_status_to_exit_status(bool_2)
    assert exit_status_4 == module_0.ExitStatus.SUCCESS
    int_1 = 432
    exit_status_5 = module_0.http_status_to_exit_status(int_1)
    assert exit_status_5 == module_0.ExitStatus.ERROR_HTTP_4XX
    none_type_0 = None
    module_0.http_status_to_exit_status(none_type_0)


def test_case_5():
    bool_0 = False
    exit_status_0 = module_0.http_status_to_exit_status(bool_0)
    assert exit_status_0 == module_0.ExitStatus.SUCCESS
    assert module_0.ExitStatus.SUCCESS == module_0.ExitStatus.SUCCESS
    assert module_0.ExitStatus.ERROR == module_0.ExitStatus.ERROR
    assert module_0.ExitStatus.ERROR_TIMEOUT == module_0.ExitStatus.ERROR_TIMEOUT
    assert module_0.ExitStatus.ERROR_HTTP_3XX == module_0.ExitStatus.ERROR_HTTP_3XX
    assert module_0.ExitStatus.ERROR_HTTP_4XX == module_0.ExitStatus.ERROR_HTTP_4XX
    assert module_0.ExitStatus.ERROR_HTTP_5XX == module_0.ExitStatus.ERROR_HTTP_5XX
    assert (
        module_0.ExitStatus.ERROR_TOO_MANY_REDIRECTS
        == module_0.ExitStatus.ERROR_TOO_MANY_REDIRECTS
    )
    assert module_0.ExitStatus.PLUGIN_ERROR == module_0.ExitStatus.PLUGIN_ERROR
    assert module_0.ExitStatus.ERROR_CTRL_C == module_0.ExitStatus.ERROR_CTRL_C
    int_0 = 599
    exit_status_1 = module_0.http_status_to_exit_status(int_0)
    assert exit_status_1 == module_0.ExitStatus.ERROR_HTTP_5XX
    bool_1 = True
    int_1 = 4250
    exit_status_2 = module_0.http_status_to_exit_status(int_1)
    assert exit_status_2 == module_0.ExitStatus.SUCCESS
    exit_status_3 = module_0.http_status_to_exit_status(bool_1)
    assert exit_status_3 == module_0.ExitStatus.SUCCESS
    int_2 = -523
    exit_status_4 = module_0.http_status_to_exit_status(int_2, int_0)
    assert exit_status_4 == module_0.ExitStatus.SUCCESS
    int_3 = 432
    exit_status_5 = module_0.http_status_to_exit_status(int_3)
    assert exit_status_5 == module_0.ExitStatus.ERROR_HTTP_4XX
    exit_status_6 = module_0.http_status_to_exit_status(exit_status_3, int_1)
    assert exit_status_6 == module_0.ExitStatus.SUCCESS


@pytest.mark.xfail(strict=True)
def test_case_6():
    bool_0 = False
    exit_status_0 = module_0.http_status_to_exit_status(bool_0)
    assert exit_status_0 == module_0.ExitStatus.SUCCESS
    assert module_0.ExitStatus.SUCCESS == module_0.ExitStatus.SUCCESS
    assert module_0.ExitStatus.ERROR == module_0.ExitStatus.ERROR
    assert module_0.ExitStatus.ERROR_TIMEOUT == module_0.ExitStatus.ERROR_TIMEOUT
    assert module_0.ExitStatus.ERROR_HTTP_3XX == module_0.ExitStatus.ERROR_HTTP_3XX
    assert module_0.ExitStatus.ERROR_HTTP_4XX == module_0.ExitStatus.ERROR_HTTP_4XX
    assert module_0.ExitStatus.ERROR_HTTP_5XX == module_0.ExitStatus.ERROR_HTTP_5XX
    assert (
        module_0.ExitStatus.ERROR_TOO_MANY_REDIRECTS
        == module_0.ExitStatus.ERROR_TOO_MANY_REDIRECTS
    )
    assert module_0.ExitStatus.PLUGIN_ERROR == module_0.ExitStatus.PLUGIN_ERROR
    assert module_0.ExitStatus.ERROR_CTRL_C == module_0.ExitStatus.ERROR_CTRL_C
    exit_status_1 = module_0.http_status_to_exit_status(bool_0)
    assert exit_status_1 == module_0.ExitStatus.SUCCESS
    int_0 = 4250
    int_1 = 396
    exit_status_2 = module_0.http_status_to_exit_status(int_1, int_0)
    assert exit_status_2 == module_0.ExitStatus.SUCCESS
    bool_1 = False
    exit_status_3 = module_0.http_status_to_exit_status(bool_1, exit_status_2)
    assert exit_status_3 == module_0.ExitStatus.SUCCESS
    none_type_0 = None
    module_0.http_status_to_exit_status(none_type_0)
